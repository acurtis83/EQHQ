...
-- ...and superseded further down. Categories moved into a table so the
-- presidency can add their own, and a constraint naming four of them by hand
-- would reject the fifth. The post-categories block at the end of this file
-- drops it again, and runs after this, so a fresh database ends up without
-- it. Left in place because an older database still needs this rewrite to
-- happen in the order it originally did.
...
-- ==== post categories (see supabase/categories.sql) ====
...
-- ==== the teaching schedule, readable by members (see supabase/teaching-public.sql) ====
