  ...
  return (
    <input
      ...
      style={{
        ...
        padding: "9px 11px", fontSize: 15, color: T.ink, width: "100%",
...
export function Chip({ children, color = T.primaryDeep, bg = T.primarySoft }) {
  return (
    <span style={{
      background: bg, color, fontSize: 12, fontWeight: 700, padding: "3px 9px",
...
export function SectionTitle({ children, sub }) {
  return (
    <div style={{ marginBottom: 12 }}>
      <div style={{ fontSize: 19, fontWeight: 700, color: T.ink, letterSpacing: "-0.01em" }}>{children}</div>
...
export function Empty({ title, hint }) {
  return (
    <div style={{ ...card, textAlign: "center", padding: "34px 18px" }}>
      <div style={{ fontSize: 15.5, fontWeight: 600, color: T.ink }}>{title}</div>
      {hint && <div style={{ fontSize: 13.5, color: T.sub, marginTop: 6, lineHeight: 1.55 }}>{hint}</div>}
    </div>
  );
}
...
export function Stub({ title, note }) {
  return (
    <div style={{ ...card, borderStyle: "dashed" }}>
      ...
      <div style={{ fontSize: 13.5, color: T.sub, marginTop: 6, lineHeight: 1.6 }}>{note}</div>
