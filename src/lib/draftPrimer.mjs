...
//
// The work itself lives here rather than in the function file, because two
// functions need it and a Netlify function cannot be both. Adding a schedule
// to a function makes it schedule-only: Netlify answers 403 to any HTTP
// request for it, which is right — a scheduled job shouldn't be triggerable
// by anyone who guesses the URL — but it means the documented ?dry=1 test
// could never have worked in production. So the schedule is one wrapper and
// the hand-triggered endpoint is another, both calling run() below.
//
...
import { toIso, isScheduledSunday, sundaysBetween } from "./domain/dates.js";
import { primerColumns, isEmptyPrimer, primerFromRow } from "./domain/primer.js";
import { talkBody, looksLikeTalk } from "./domain/talkText.js";
import { SYSTEM, primerPrompt, readReply } from "./domain/primerPrompt.js";
